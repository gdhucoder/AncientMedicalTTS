...
import { annotationForToken, annotationSourceLabel, confidenceLabel, isHanToken, isSingleHanGlobalRule, isTtsLockedAnnotation, normalizePinyinInput, realizedPronunciationForText, reviewStatusLabel, riskTypeLabel, ruleTypeLabel, ttsPronunciationMismatches } from "./services/pronunciationUi";
...
function ReaderPage() {
  ...
  const currentAudio = reader?.audio_versions.find((version) => version.id === reader.segment.current_audio_id) ?? null;
  const expectedPinyin = reader && displaySettings.pinyin_mode === "all" && displayPinyin
    ? reader.tokens.map((token) => {
      const annotation = reader.annotations.find((item) => token.index >= item.start_token && token.index < item.end_token);
      if (annotation) {
        const ttsLocked = isTtsLockedAnnotation(annotation);
        const annotationPinyin = annotation.review_status === "ignored" ? null : ttsLocked ? annotation.target_pinyin : annotation.default_pinyin ?? annotation.candidate_pinyin[0] ?? null;
        const fallbackPinyin = displayPinyin.slice(annotation.start_token, annotation.end_token).filter((value): value is string => value !== null).join(" ") || null;
        const shownPinyin = annotationPinyin ?? fallbackPinyin;
        return shownPinyin?.trim().split(/\s+/)[token.index - annotation.start_token] ?? null;
      }
      return displayPinyin[token.index] ?? null;
    })
    : null;
  const ttsMismatches = reader && expectedPinyin ? ttsPronunciationMismatches(reader.tokens, expectedPinyin, currentAudio) : new Set<number>();
  const ttsComparisonAvailable = displaySettings.pinyin_mode === "all"
    && displayPinyin !== null
    && (currentAudio?.provider_metadata?.realized_pronunciation ?? []).some((item) => item.text.trim() && item.phoneme?.trim());
          ...
          <section className="reader-workspace">
            ...
            {reader ? <>
              ...
              if (annotation) {
                ...
                const shownPinyin = annotationPinyin ?? fallbackPinyin;
                const pronunciationLabel = ttsLocked ? "已锁定读音 · 将用于语音合成" : "参考注音 · 仅用于页面显示";
                const hasTtsMismatch = Array.from({ length: annotation.end_token - annotation.start_token }, (_, offset) => annotation.start_token + offset).some((index) => ttsMismatches.has(index));
                const content = shownPinyin ? <ruby className={`${ttsLocked ? "pronunciation-ruby locked" : "pronunciation-ruby preview"}${hasTtsMismatch ? " tts-mismatch" : ""}`}><span>{text}</span><rt>{pinyinToToneMarks(shownPinyin)}</rt></ruby> : text;
                return <button key={annotation.id} className={`annotation-token status-${annotation.review_status}${selectedAnnotationId === annotation.id ? " selected" : ""}${cursorTokenIndex === annotation.start_token ? " cursor-token" : ""}${hasTtsMismatch ? " tts-mismatch" : ""}`} type="button" onClick={() => selectAnnotation(annotation)} title={`${riskTypeLabel(annotation.risk_type)} · ${reviewStatusLabel(annotation.review_status)} · ${pronunciationLabel}${hasTtsMismatch ? " · TTS实际发音与全文注音不一致" : ""}`}>{content}</button>;
              }
              ...
              const hasTtsMismatch = ttsMismatches.has(token.index);
              const content = tokenPinyin ? <ruby className={`pronunciation-ruby preview${hasTtsMismatch ? " tts-mismatch" : ""}`}><span>{token.text}</span><rt>{pinyinToToneMarks(tokenPinyin)}</rt></ruby> : token.text;
              return <button key={token.index} className={`${manualTokenIndex === token.index ? "plain-token selected" : "plain-token"}${cursorTokenIndex === token.index ? " cursor-token" : ""}${hasTtsMismatch ? " tts-mismatch" : ""}`} type="button" onClick={() => selectManualToken(token.index)} title={hasTtsMismatch ? "TTS实际发音与全文注音不一致" : undefined}>{content}</button>;
                })}</div>
