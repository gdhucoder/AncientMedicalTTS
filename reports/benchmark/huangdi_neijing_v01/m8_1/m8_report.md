...
- 漏报 5；wrong default 0；candidate missing 0；extra predictions 69（每千汉字 20.8）。
...
| extra predictions | 1696 | 69 |

M8 验收目标仍为 detection recall ≥ 85%、default accuracy ≥ 80%、candidate coverage ≥ 98%、candidate missing = 0；本次全部达到。上一轮 79 个 extra 中有 10 个属于通用默认拼音归一化 bug，修复后剩余 69 个；没有把 Gold 写入生产路径。
...
|---|---:|---:|
| context_rule_v2 | 50 | 9 |
| high_risk_polyphone | 48 | 48 |
| medical_lexicon | 61 | 12 |
| rare_character | 6 | 0 |
| variant_mapping | 2 | 0 |
...
Extra risk_type 分布：context_pronunciation=9, medical_term=12, polyphone=48。
...
| 上古天真论篇第一 | 15 | 藏 | cang2 | polyphone | high_risk_polyphone | useful | 命中版本化高风险多音字表，属于可解释的复核提示 | 竭，精少，肾藏衰，形体皆极；
...
| 四气调神大论篇第二 | 4 | 藏 | cang2 | polyphone | high_risk_polyphone | useful | 命中版本化高风险多音字表，属于可解释的复核提示 | 气平，无外其志，使肺气清。此秋气之应养收之道也；逆之则伤肺，冬为飧泄，奉藏者少。
...
本轮重跑：extra predictions 69；全量分类统计：{'bug': 0, 'unclear': 0, 'unnecessary': 0, 'useful': 69}。本轮完整分类及理由见 `m8_extra_predictions.csv`；修复前 79 条的全量审计见 `m8_1_pre_fix_extra_predictions.csv` 及对应 JSON 统计文件。
...
漏报原因分类（工程归因）：{'text_variant': 1, 'rare_character_not_detected': 2, 'classical_usage_not_detected': 2}。Gold occurrence 跨 Segment 边界 0 个；本数据集 supplementary-plane CJK 字符问题记录为 False。这些结论仅针对本数据集。
